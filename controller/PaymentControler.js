...
class PaymentControler {
    ...
    static paymentInvoice = catchAsync(async (req, res) => {
        ...
        const html = await ejs.renderFile(templatePath, { /* your invoice data */ });

        // 2. Send HTML to PDF API (No Chrome required on your server)
        const response = await axios.post(
            'https://v2.api2pdf.com/chrome/pdf/html',
            { html: html, inline: true },
            { headers: { 'Authorization': 'YOUR_API2PDF_KEY' }, responseType: 'arraybuffer' }
        );

        const pdfBuffer = Buffer.from(response.data);

        // 3. Return PDF stream to client
        res.set({
